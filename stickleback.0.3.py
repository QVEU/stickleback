...
v0.3 version notes:
    Includes new cutoff with minimum 1 distance.
    Chunking reads to avoid OOM in poolBlocks() 
'''
...
def poolBlocks(query,pdSam,nthreads=20): #uses multithreading to compute the matches (Edited to 14 threads 4/27/23 WB)
    pdSamOut=pd.DataFrame()#All new to chunk out mapping to avoid OOM failures - PTD 5/22/26
    n = int(1E5)  #step interval
    a = range(0,len(pdSam),n) #number of reads
    a=list(a)
    a.append(len(pdSam))
    print(a)
    res = [] 
    lastRow=0
    for rows in a[1:]:
        tempSam=pd.DataFrame(pdSam[lastRow:rows])
        lastRow=rows
        #print(tempSam)
        with Pool(nthreads) as p:
            print("\n1. Computing minimum distance hit position for {} reads.".format(len(tempSam)))
            quL=len(query)
            print("Query: {}".format(query))
            tempSam['minD'],   tempSam['minPos'],   tempSam['matchseq'],    tempSam['context'],   tempSam['matchlist']   = zip(*p.map(blockDist, [(c.upper(),query.upper()) for c in tempSam.seq]))
